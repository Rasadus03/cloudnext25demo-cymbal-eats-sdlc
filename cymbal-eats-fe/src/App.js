  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        ...
        const response = await fetch('https://cymbal-eats.org//restaurants-api/restaurants', {mode: 'no-cors'}); //  Use a relative path
...
// Toggle Sign in/out button
  const  toggle = async () => {
    ...
    if (customer instanceof Map) {
      // Replace with your actual API endpoint
      const response2 = await fetch('https://cymbal-eats.org//shopping-cart-api/get-cart-item-count?user-id=' +customer.get("email"), {mode: 'no-cors'}); //  Use a relative path
...
*/
  ...
  const addToCart = async (restaurantId, item) => {
    ...
    fetch("https://cymbal-eats.org//shopping-cart-api/add-shopping-cart-item", {
      method: "POST",
      headers: {
    ...
    const removeFromCart = (restaurantId, item, setCart, shoppingCartItems) => {
      if (customer instanceof Map) {
        setCartItems(cartItems.filter((item2) => item2.menuItemId !== item.menuItemId));
        setCart(shoppingCartItems.filter((item2) => item2.menuItemId !== item.menuItemId));
        ...
        console.log("shoppingItem " + JSON.stringify(shoppingItem));
        fetch(
            "https://cymbal-eats.org//shopping-cart-api/remove-shopping-cart-item",
            {
              method: "POST",
              headers: {
                "Content-Type": "Application/JSON",
  ...
  const updateQuantity = (restaurantId, item, newQuantity, setCart, shoppingCartItems) => {
    if (customer instanceof Map) {
      ...
      setCart(shoppingCartItems.map((item2) =>
          item2.menuItemId === item.menuItemId ? {...item2, quantity: newQuantity} : item2
      ));
      shoppingCartItems.map((item2) =>
          item2.menuItemId === item.menuItemId ? console.log("updated item" + item2) : item2);
      const shoppingItem = {
        //restaurantName: getRestaurantName(restaurantId),
        restaurantId: restaurantId,
        menuItemId: item.menuItemId,
      ...
      console.log("shoppingItem " + JSON.stringify(shoppingItem));
      fetch(
          "https://cymbal-eats.org//shopping-cart-api/update-shopping-cart-item-quantity",
          {
            method: "POST",
            headers: {
              "Content-Type": "Application/JSON",
  ...
  const clearCart = (userId, setCart) => {
    if (customer instanceof Map) {
      ...
      fetch("https://cymbal-eats.org//shopping-cart-api/clear-shopping-cart", {
        method: "POST",
        headers: {
          "Content-Type": "Application/JSON",
        },
